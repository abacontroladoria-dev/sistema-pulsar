...
const CAIXAS_UI: Record<Caixa, DefCaixa> = {
  maia: {
    ...
    icon: Bot, texto: 'text-violet-700 dark:text-violet-200', borda: 'border-border',
    fundo: 'bg-card', realce: false,
  },
  humano: {
    id: 'humano', label: 'Humano', ajuda: 'Alguém da equipe assumiu',
    ordem: 'quem espera há mais tempo primeiro',
    icon: User, texto: 'text-emerald-700 dark:text-emerald-200', borda: 'border-border',
    fundo: 'bg-card', realce: false,
  },
  ninguem: {
    id: 'ninguem', label: 'Ninguém', ajuda: 'A Maia saiu e nenhum humano pegou',
    ordem: 'quem espera há mais tempo primeiro',
    icon: AlertTriangle, texto: 'text-amber-700 dark:text-amber-200', borda: 'border-amber-500/40',
  ...
  encerradas: {
    ...
    icon: Archive, texto: 'text-muted-foreground', borda: 'border-border',
    fundo: 'bg-card', realce: false,
  },
}
    ...
    <div className="h-full flex flex-col bg-background text-foreground">
      <header className="px-6 pt-6 pb-4 border-b border-border shrink-0">
        ...
        {/* O padrão da clínica desligado explica uma coluna "Maia" vazia. Sem
            ...
            no lugar errado. */}
        {modoPadrao === 'off' && (
          <div className="mt-3 flex items-start gap-2 rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-sm text-amber-700 dark:text-amber-200">
        ...
        {erro && (
          <div className="mt-3 flex items-center gap-2 rounded-lg border border-rose-500/30 bg-rose-500/10 px-3 py-2 text-sm text-rose-700 dark:text-rose-200">
  ...
  return (
    ...
    >
      <header className="px-4 py-3 border-b border-border shrink-0">
        ...
        <p className="text-[11px] leading-tight text-muted-foreground/70 mt-1">{def.ajuda}</p>
        <p className="text-[11px] leading-tight text-muted-foreground/70 mt-0.5">{def.ordem}</p>
      </header>

      <div className="flex-1 min-h-0 overflow-auto p-2 space-y-1.5">
        {fila.conversas.length === 0 ? (
          <p className="text-xs text-muted-foreground/70 text-center py-8">Nenhuma conversa.</p>
        ) : (
      ...
      {ocultas > 0 && (
        ...
        <footer className="px-4 py-2 border-t border-border text-[11px] text-muted-foreground/70 shrink-0">
...
const LinhaConversa: React.FC<{
  ...
  return (
    ...
    <Link
      ...
      className="block rounded-lg border border-border bg-card px-3 py-2.5 hover:border-border hover:bg-card transition"
    >
      <div className="flex items-center gap-2">
        ...
        <span
          className={`ml-auto text-sm font-semibold tabular-nums shrink-0 ${
            critica ? 'text-amber-700 dark:text-amber-200' : 'text-muted-foreground'
          }`}
        >
      ...
      <div className="flex items-center gap-2 mt-1 pl-9">
        ...
        {/* A escalada da Maia que ninguém pegou. Não é coluna própria (decisão do
            ...
            socorro. */}
        {urgente && (
          <span className="ml-auto px-1.5 py-0.5 rounded text-[10px] font-medium bg-amber-500/20 text-amber-700 dark:text-amber-200 border border-amber-500/30 shrink-0">
            escalada
          </span>
        )}
      </div>
