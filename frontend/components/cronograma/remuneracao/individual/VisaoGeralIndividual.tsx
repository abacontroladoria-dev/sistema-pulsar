...
function Card({ titulo, icone, children, className = "" }: {
  titulo: string; icone: React.ReactNode; children: React.ReactNode; className?: string
}) {
  return (
...
function BarraH({ fracao, cor }: { fracao: number; cor: string }) {
...
function BarraEmpilhada({ partes }: { partes: { valor: number; cor: string; rotulo: string }[] }) {
...
function Legenda({ cor, rotulo, valor }: { cor: string; rotulo: string; valor: number }) {
  return (
    <span className="inline-flex items-center gap-1.5 text-xs text-muted-foreground">
      ...
      {rotulo}
      <span className="font-bold tabular-nums text-foreground">{num(valor)}</span>
    </span>
  )
}
