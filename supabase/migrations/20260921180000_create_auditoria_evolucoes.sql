-- ============================================================================
-- MIGRATION: 20260921180000_create_auditoria_evolucoes.sql
-- Auditoria de Evoluções Terapêuticas (IA & Anti-Glosa)
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS public.auditoria_evolucoes (
    ...
    historico_cobranca JSONB DEFAULT '[]'::jsonb, -- [{ data: string, usuario: string, acao: string, observacao?: string }]
...
COMMENT ON TABLE public.auditoria_evolucoes IS 'Resultados da auditoria clínica de evoluções terapêuticas via IA e controle de cobrança operacional.';
COMMENT ON COLUMN public.auditoria_evolucoes.status_risco IS 'sem_risco (conforme), risco_especifico (ajuste menor), risco_relevante (alto risco de glosa de convênio).';
COMMENT ON COLUMN public.auditoria_evolucoes.status_cobranca IS 'pendente (não cobrado), cobrado (notificado), aguardando_correcao, corrigido_tita, ignorado (dispensado).';
