...
--
-- `rls_ligada` PRECISA vir true. Se vier false, a tabela está aberta a qualquer
-- usuário autenticado de QUALQUER organização (o event trigger rls_auto_enable
-- não cobre o schema `central`), e o `alter table ... enable row level security`
-- acima precisa ser executado à mão antes de qualquer uso.
-- ============================================================================
select
  c.relname                                   as tabela,
  c.relrowsecurity                            as rls_ligada,
  (select count(*) from pg_policies p
    where p.schemaname = 'central' and p.tablename = 'tasks') as politicas
from pg_class c
join pg_namespace n on n.oid = c.relnamespace
where n.nspname = 'central' and c.relname = 'tasks';
