-- =============================================================================
-- APLICAR no SQL Editor — 20260911120000_biofacial_na_auditoria_diaria
-- =============================================================================
-- Cópia fiel de supabase/migrations/20260911120000_biofacial_na_auditoria_diaria.sql,
-- envolvida em UMA transação. Rode o arquivo INTEIRO de uma vez.
--
-- POR QUE A TRANSAÇÃO IMPORTA: as duas funções têm de trocar de assinatura
-- JUNTAS. `get_auditoria_assim` é `SELECT * FROM get_auditoria_assim_periodo`,
-- então entre um CREATE e o outro o wrapper fica incompatível com a função
-- interna e a tela diária responde "structure of query does not match function
-- result type". DDL em Postgres é transacional: dentro do BEGIN/COMMIT essa
-- janela não existe para ninguém.
--
-- SE O SQL EDITOR RECUSAR O TAMANHO (ele derruba comando longo com um "Failed
-- to fetch" que é do NAVEGADOR, não do banco —
-- reference_sql_editor_derruba_comando_longo): rode em duas execuções NA MESMA
-- SESSÃO e em sequência imediata — primeiro a função de período, depois o
-- wrapper + GRANTs. A janela de incompatibilidade volta a existir, então faça
-- isso fora do horário de atendimento.
--
-- DEPOIS: rodar 20260911_registrar_biofacial_auditoria_no_livro_caixa.sql.
-- NUNCA `supabase db push` (reference_db_push_blast_radius).
