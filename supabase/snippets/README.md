...
| `20260910_APLICAR_tv_avisos_e_papel_marketing.sql` | 6 migrations do carrossel de avisos da TV: `20260831150000` (tabela `tv_avisos`, RLS, bucket `tv-avisos` público em leitura), `20260831150100` (seed da permissão `tv_avisos`), `20260910120000` (papel `marketing`), `20260910120100` (RLS/policies do bucket passam a aceitar o papel `marketing`), `20260910120200` (permissão muda de grupo visual para 'Marketing') e `20260910120300` (grupo de permissões semente 'Marketing'). Nenhuma das 6 tinha entrado em produção — foi por isso que duas imagens de teste subidas em `/tv-avisos` não apareceram na TV mesmo após F5: tabela/bucket não existiam. ⚠ Bloco do bucket e as policies de `storage.objects`: se o SQL Editor recusar com "must be owner of table objects", recriar pelo Dashboard (Storage > tv-avisos > Policies) com as mesmas expressões | aplicado 2026-09-10 |
