-- Conferência de Guias (/conferencia-guias) — 24/09/2026
--
-- APLICA a migration 20260924200000_conferencia_guias_assinadas numa transação
-- e registra a versão no livro-caixa (supabase_migrations.schema_migrations),
-- para um `db push` futuro não tentar reaplicá-la.
--
-- Por que snippet e não `db push`: o push empurra TODO o pendente, não só esta
-- (ver reference_db_push_blast_radius).
--
-- Só CRIA coisas novas (tabela, função, uma linha em permissoes): não toca em
-- nenhuma tabela quente, então não precisa de lock_timeout. Reexecutável.
...
values ('20260924200000', 'conferencia_guias_assinadas')
on conflict (version) do nothing;

commit;

-- Conferência (rode depois):
select 'livro-caixa' as item, count(*)::text as valor from supabase_migrations.schema_migrations where version = '20260924200000'
union all
select 'tabela', count(*)::text from pg_tables where schemaname = 'public' and tablename = 'conferencia_guias_assinadas'
union all
select 'policies', count(*)::text from pg_policies where tablename = 'conferencia_guias_assinadas'
union all
select 'rpc', count(*)::text from pg_proc where proname = 'get_conferencia_guias_dia'
union all
select 'catálogo', count(*)::text from public.permissoes where codigo = 'conferencia_guias';
-- Esperado: 1, 1, 1, 1, 1.
